...
class Anuncios
{
    ...
    public function getTotalAnuncios($filtros)
    {
        ...
        /*
         * Preenchendo os binds do meu array
         */
        if(!empty($filtros['categoria'])) {
    ...
    // filtrando meus ultimos anuncios
    public function getUltimosAnuncios ($page, $perPage, $filtros)
    {
        global $pdo;

        // Minha Paginaçao sobre os anuncios
    ...
    /*
     * Metodo responsavel por retornar meus anuncioas e as imagens atreladas
     */
    ...
    /*
     * Metodo para a inserçao de anuncio
     */
    ...
    /*
     * Metodo que realiza a exclusao do anuncio
     */
    ...
    /*
     * Metodo para retornar o anuncio por id
     */
    ...
    /*
     * Metodo para a edicao do anuncio
     */
    public function eddAnuncio ($titulo, $categoria, $valor, $descricao, $estado, $fotos, $id)
    {
        global $pdo;
        ...
        // verificando se foi enviada a foto
        if(count($fotos) > 0) {
            // realizando um laço sobre as minhas fotos
            for($q=0;$q<count($fotos['tmp_name']);$q++) {
                // minha variavel "type" verifica se e uma imagem
                $tipo = $fotos['type'][$q];
                if(in_array($tipo, array('image/jpeg', 'image/png'))) {
                    // criando um hash md5 para a nomeaçao na minha imagem
                    $tmpname = md5(time().rand(0,9999)).'.jpg';
                    // movendo a minha imagem para a pasta anuncios com o nome do hash
                    move_uploaded_file($fotos['tmp_name'][$q], 'assets/images/anuncios/'.$tmpname);

                    // Pegando a altura e largura da minha imagem original
                    list($width_orig, $height_orig) = getimagesize('assets/images/anuncios/'.$tmpname);
                    //largura original dividido por altura original
                    $ratio = $width_orig/$height_orig;

                    $width = 500;
                    $height = 500;

                    // quer dizer que minha imagem original nao esta na propoerçao desejada de 500
                    ...
                    // verificando se minha imagem sera jpg ou png
                    $img = imagecreatetruecolor($width, $height);
                    // caso jpg salvo a mesma no servidor
                    if($tipo == 'image/jpeg') {
                        $origi = imagecreatefromjpeg('assets/images/anuncios/'.$tmpname);
                        // caso png salvo a mesma no servidor
